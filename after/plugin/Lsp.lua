...
local lua_snip = require("luasnip.loaders.from_vscode").lazy_load({ exclude = {} })
...
vim.lsp.config("typos_lsp", { cmd = { "typos-lsp", "--exclude=*.tex", "--exclude=*.txt", "--locale=en-gb" } })
...
--require('java').setup({verification = { invalid_mason_registry = false }})
...
local function toggle_lsp_server(name)
	...
	for _, client in pairs(buf_clients) do
		if client.name == name and not client.is_stopped() then
			...
			message = message .. " Stopping " .. vim.inspect(client.name) .. ":" .. client.id .. ":" .. namspace
		end
	end
	print(message)
	if not found then
...
vim.api.nvim_create_autocmd("LspAttach", {
	callback = function(args)
		...
		vim.keymap.set("n", "<C-k>", function()
			vim.lsp.buf.signature_help(floating)
		end, opts)
		...
		-- Char 46 is '.'
		vim.keymap.set("n", "<Char-46>", function()
			vim.lsp.buf.code_action()
		end, opts)
...
--vim.lsp.enable({ name = "jdtls", enable = false })
